...
public class AutoSuggestService {
	...
	public static String MED_INDEX_PATH = "/var/www/spedia/medical";
	...
	public static Set<Map> getCityDocument(Map<String,String> query, int hitCount) throws IOException, ParseException {
		IndexReader reader = DirectoryReader.open(FSDirectory
				.open(new File(SchoolsConstants.INDEX_PATH)));
		searcher = new IndexSearcher(reader);
		QueryBuilder queryBuilder = new QueryBuilder(getAnalyzer());
		...
		if (!SocialUtility.chkNull(term)) {
			term = parseQueryString(term);
			q1 = new QueryParser(Version.LUCENE_CURRENT,"city", getAnalyzer()).parse(term);
			booleanQuery.add(q1, BooleanClause.Occur.MUST);
		}
		String state = query.get("state");
		if (!SocialUtility.chkNull(state)) {
	...
	public static Set<String>  getMedical(String query, int hitCount) throws IOException, ParseException {
		IndexReader reader = DirectoryReader.open(FSDirectory
				.open(new File(MED_INDEX_PATH)));
		searcher = new IndexSearcher(reader);
		QueryBuilder queryBuilder = new QueryBuilder(getAnalyzer());
		BooleanQuery booleanQuery = new BooleanQuery();
		Query q1=null;
		if (!SocialUtility.chkNull(query)) {
			query = parseQueryString(query);
			q1 = new QueryParser(Version.LUCENE_CURRENT,"name", getAnalyzer()).parse(query);
			booleanQuery.add(q1, BooleanClause.Occur.MUST);
		}
		...
		try {
			...
			for (int i = 0; i < hits.length; ++i) {
				...
				int docId = hits[i].doc;
				Document d = searcher.doc(docId);
				String r=d.get("name") +" (unit:"+d.get("unit") +") price:"+d.get("price");
				result.add(r);
	...
	public static void main(String[] args) {
		...
		try {
			...
			d = getCityDocument(query, hitCount);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();

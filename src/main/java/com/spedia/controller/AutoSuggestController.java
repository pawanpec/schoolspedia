...
public class AutoSuggestController extends BaseWSController{
	...
	public @ResponseBody String autoSuggestCompanies(HttpServletRequest req,HttpSession session) throws Exception {
		...
		if(!SocialUtility.chkNull(state)){
			query.put("state", state.toLowerCase());
		}
	...
	@RequestMapping(value = "/autoSuggestCity.json", method = RequestMethod.GET)
	public @ResponseBody String autoSuggestCity(HttpServletRequest req,HttpSession session) throws Exception {
		Map<String,String> query=new HashMap<String,String>();
		String state=req.getParameter("state");
		String term=req.getParameter("term");
		if(!SocialUtility.chkNull(term)){
		...
		int hitCount=10;
		Set<Map> d =autoSuggestService.getCityDocument(query, hitCount);
		return convertIntoJson(d);
	}
	@RequestMapping(value = "/autoSuggestMed.json", method = RequestMethod.GET)
	public @ResponseBody String autoSuggestMed(HttpServletRequest req,HttpSession session) throws Exception {
		Map<String,String> query=new HashMap<String,String>();
		String term=req.getParameter("q");
		int hitCount=50;
		Set<String> d =autoSuggestService.getMedical(term,hitCount);
		return convertIntoJson(d);
	}
}
